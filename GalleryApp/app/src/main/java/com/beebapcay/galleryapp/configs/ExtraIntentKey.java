package com.beebapcay.galleryapp.configs;

public class ExtraIntentKey {
	public static String PACKAGE_NAME = "com.beebapcay.galleryapp";
	public static String EXTRA_TITLE = PACKAGE_NAME + "EXTRA_TITLE";
	public static String EXTRA_OPTION_GALLERY_LIST_TYPE = PACKAGE_NAME + "EXTRA_OPTION_GALLERY_LIST_TYPE";
	public static String EXTRA_ALBUM_DATA = PACKAGE_NAME + "EXTRA_ALBUM_DATA";
	public static String EXTRA_HERO_ITEM_TYPE = PACKAGE_NAME + "EXTRA_HERO_ITEM_TYPE";
	public static String EXTRA_HERO_ITEM_DATA = PACKAGE_NAME + "EXTRA_HERO_ITEM_DATA";
}
